...
void SymProc::objDestroy(TObjId obj)
{
    ...
    // FIXME: this will unconditionally plot heap of leaked memory shape
    // with ::debuggingGarbageCollector without printing a warning first
    lm.leave();
}
        ...
        const bool                   allowOverlap)
{
    ...
    {
        CL_ERROR_MSG(loc, "string literal not supported as source in call of "
                << fnc);
        proc.printBackTrace(ML_ERROR);
        return;
    }
        ...
        const TSizeRange                size)
{
    ...
    if (!size.hi) {
        // simulate realloc(ptr, 0) as { free(ptr); return NULL; }
        SymHeap nullHeap(sh_);
        SymExecCore nullCore(nullHeap, bt_, ep_);
        nullCore.setLocation(lw_);
        Trace::waiveCloneOperation(nullHeap);

        // free memory and return NULL
        const FldHandle nullLhs(nullHeap, lhs);
        nullCore.execFree(valAddr, /* reallocated */ true);
        nullCore.setValueOf(nullLhs, VAL_NULL);
        nullCore.killInsn(insn);
        dst.insert(nullHeap);
    }
